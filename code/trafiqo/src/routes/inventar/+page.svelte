<script>
    ...
    let neuFehler = $state("");
    let neuArtikel = $state({ artikelnr: "", bezeichnung: "", preis: 0, alter: 0, katId: 1, liefId: 1 });
    ...
    async function speichereNeuenArtikel() {
        ...
        try {
            ...
            if (result.p_status === 'ERROR') {
                ...
                zeigeNeuPopup = false;
                eingangStatus = { type: 'success', msg: `Artikel ${neuArtikel.bezeichnung} erfolgreich angelegt.` };
                await ladeInventar();
                neuArtikel = { artikelnr: "", bezeichnung: "", preis: 0, alter: 0, katId: 1, liefId: 1 };
            }
        } catch (e) {
    ...
    async function handleWareneingang(event) {
        if (event.key === 'Enter' && eingabeArtikelNr.trim() !== '') {
            ...
            try {
                const result = await api.buchenWareneingang(eingabeArtikelNr.trim(), eingabeMenge);
                
                if (result.p_status === 'ERROR') {
                    eingangStatus = { type: 'error', msg: result.p_error };
                } else {
                    const index = artikelListe.findIndex(a => a.artikelnr === eingabeArtikelNr.trim());
                    let berechneterNeuerBestand = 0;

                    if (index !== -1) {
                        berechneterNeuerBestand = Number(artikelListe[index].lagerbestand) + Number(eingabeMenge);
                        artikelListe[index].lagerbestand = berechneterNeuerBestand;
                    } else {
                        berechneterNeuerBestand = result.p_neuer_bestand || "unbekannt";
                    }
                ...
                eingangStatus = { type: 'error', msg: "Netzwerkfehler: " + e.message };
            }
        }
    }
</script>
...
<div class="inventar-container">
    ...
    <div class="wareneingang-panel">
        ...
        <p class="panel-hint">Scannen Sie den Barcode der Lieferung. Die Menge wird automatisch dem Lager hinzugefugt.</p>
        
        <div class="scanner-reihe">
            <div class="input-group compact">
                <label for="menge">Menge</label>
                <input id="menge" type="number" min="1" bind:value={eingabeMenge} />
            </div>
                ...
                <table>
                    ...
                    <tbody>
                        {#each artikelListe as artikel}
                            <tr class={artikel.lagerbestand < 5 ? 'low-stock' : ''}>
                                <td class="mono">{artikel.artikelnr}</td>
                                <td class="article-name">{artikel.bezeichnung}</td>
                                <td class="text-right">{artikel.verkaufspreis.toFixed(2)} EUR</td>
                                <td class="text-center">
                                    <span class="stock-badge {artikel.lagerbestand < 5 ? 'critical' : artikel.lagerbestand < 10 ? 'low' : 'normal'}">
                                        {artikel.lagerbestand}
                                    </span>
                                </td>
                                <td class="text-center">
                                    {#if artikel.altersfreigabe > 0}
                                        <span class="age-badge">{artikel.altersfreigabe}+</span>
                                    {:else}
    ...
    <div class="modal-overlay">
        <div class="modal-box">
            ...
            <div class="modal-body">
                <div class="form-grid">
                    <div class="input-group full-width">
                        ...
                        <input type="text" bind:value={neuArtikel.artikelnr} use:autoFocus placeholder="Barcode scannen..." />
                    </div>
                    
                    <div class="input-group full-width">
                        <label>Bezeichnung</label>
                        <input type="text" bind:value={neuArtikel.bezeichnung} placeholder="z.B. Red Bull 0.25L" />
                    </div>
                    
                    <div class="input-group">
                        <label>Verkaufspreis (EUR)</label>
                        <input type="number" step="0.01" bind:value={neuArtikel.preis} />
                    </div>
                    
                    <div class="input-group">
                        <label>Altersfreigabe</label>
                        <select bind:value={neuArtikel.alter} class="select-input">
                    ...
                    <div class="input-group">
                        ...
                        <input type="number" bind:value={neuArtikel.katId} />
                    </div>
            ...
            <div class="modal-footer">
                <button class="btn-secondary" onclick={() => zeigeNeuPopup = false}>Abbrechen</button>
                <button class="btn-primary" onclick={speichereNeuenArtikel}>Speichern</button>
            </div>
        </div>
    </div>
{/if}
